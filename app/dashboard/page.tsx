...
export default function DashboardPage() {
    ...
    const budgetPercent = stats.totalBudget > 0
        ? ((stats.remainingBudget / stats.totalBudget) * 100).toFixed(1)
        : "0"
    ...
    return (
        <div className="flex flex-col gap-6 p-6 md:p-4">
            ...
            <div className="grid gap-3 grid-cols-2 md:grid-cols-3 shrink-0">
                <Card className="shadow-sm border-l-4 border-l-blue-600">
                    <CardHeader className="flex flex-row items-center justify-between pb-1 pt-3 px-4">
                        <CardTitle className="text-[18px] font-semibold text-muted-foreground uppercase tracking-wider">
                            Total Budget
                        </CardTitle>
                        <Wallet className="h-6 w-6 text-blue-600" />
                    </CardHeader>
                    <CardContent className="px-4 pb-3">
                        <div className="text-xl md:text-4xl font-bold tracking-tight">₹{stats.totalBudget.toLocaleString()}</div>
                        <p className="text-[16px] text-muted-foreground mt-0.5 font-medium italic">Initial Purse Allocation</p>
                    </CardContent>
                </Card>
                ...
                <Card className="shadow-sm border-l-4 border-l-blue-600">
                    ...
                    <CardContent className="px-4 pb-3">
                        <div className="text-xl md:text-4xl font-bold text-green-700 tracking-tight">₹{stats.remainingBudget.toLocaleString()}</div>
                        <p className="text-[16px] italic text-green-600/80 mt-0.5 font-bold">{budgetPercent}% remaining</p>
                    </CardContent>
                </Card>
                ...
                <Card className="hidden md:flex bg-primary/5 border-dashed border-primary/20 flex-col justify-between">
                    <CardHeader className="pb-0 pt-4 px-4">
                        <CardTitle className="text-[18px] font-bold uppercase flex items-center gap-1.5 text-primary tracking-widest">
                            <Users className="h-4.5 w-4.5" /> Squad Total
                        </CardTitle>
                    </CardHeader>
            ...
            <div className="flex-1 min-h-0 transition-all duration-700 animate-in fade-in slide-in-from-bottom-4">
                ...
                {!isLoading && lastPutOnBlock?.action === "reset" && (
                    <Card className="h-full flex items-center justify-center border-l-4 border-l-orange-500 bg-orange-50/30 transition-all duration-700 animate-in zoom-in-95">
                        <div className="flex flex-col items-center text-center gap-6 p-8">
                            <div className="h-20 w-20 rounded-full bg-orange-100 flex items-center justify-center border-4 border-white shadow-xl">
                                <Trophy className="h-10 w-10 text-orange-600 opacity-80" />
                            </div>
                            <div className="max-w-md">
                                <h3 className="text-3xl font-black text-orange-700 uppercase italic tracking-tighter leading-none mb-4">
                                    Auction Update
                                </h3>
                                <p className="text-lg font-bold text-slate-700 uppercase tracking-tight">
                                    {lastPutOnBlock?.message || "The auction state has been updated."}
                                </p>
                            </div>
                        </div>
                    </Card>
                ...
                {!isLoading && lastPutOnBlock?.action !== "reset" && (!lastPutOnBlock || !lastPutOnBlock.playerDetails) && (
                ...
                {!isLoading && lastPutOnBlock?.action !== "reset" && lastPutOnBlock?.playerDetails && (
                    ...
                    <Card className={`h-full flex flex-col overflow-hidden shadow-sm border-l-4 bg-card transition-all duration-500 ${lastPutOnBlock?.playerDetails?.status === "sold" ? "border-l-green-600 ring-1 ring-green-600/20" :
                        lastPutOnBlock?.playerDetails?.status === "unsold" ? "border-l-red-600 ring-1 ring-red-600/20" :
                            lastPutOnBlock?.playerDetails?.status === "on_block" ? "border-l-orange-400 shadow-orange-100/50 shadow-lg" :
                                "border-l-blue-600"
                        }`}>
                            ...
                            <div className="flex flex-col gap-0.5 min-w-0">
                                <div className="flex items-center gap-2 flex-wrap">
                                    <span className={`px-2 py-0.5 rounded-full text-white text-[9px] font-black uppercase tracking-tighter transition-colors duration-500 ${lastPutOnBlock?.playerDetails?.status === "sold" ? "bg-green-600 animate-bounce" :
                                        ...
                                        {lastPutOnBlock?.playerDetails?.status === "on_block" ? "on the hammer" : lastPutOnBlock?.playerDetails?.status?.replace("_", " ") || lastPutOnBlock?.action || "Active Bid"}
                                    </span>
                                ...
                                <h2 className="pr-5 text-2xl md:text-4xl font-black tracking-tighter uppercase italic text-slate-900 truncate leading-tight">
